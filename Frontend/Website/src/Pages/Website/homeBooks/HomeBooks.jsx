...
export default function HomeBooks() {
    ...
    useEffect(() => {
        ...
        const fetchrecent = async()=>{
            try {
                const response = await axios.get('http://localhost:4000/books/recent', { withCredentials: true });
        ...
        <motion.div ref={carouselRef} className='carousel flex rounded-xl bg-white pr-6'>
            ...
            <motion.div drag="x" dragConstraints={{ right: 0, left: -width }} className='inner-carousel flex gap-5 pl-3 py-2'>
                {recentbooks.map((recentBook, index) => (
                    <img
                        key={index}
                        src={recentBook.cover}                       alt={`book-${recentBook.id}`}
                        className="h-40 w-28 mx-auto my-1 p-3 object-cover rounded-xl"
            ...
            <div className="books mt-5 mb-10">
                <section id="Projects" className="w-full md:w-fit mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 justify-items-center justify-center gap-y-5 gap-x-5">
                    {books.map((book, index) => (
                        <div key={index} className="w-full md:w-36 bg-white shadow-md rounded-lg duration-500 hover:scale-105 hover:shadow-xl">
                            <a href={`/${book.id}`}>
                                <img
                                    src={book.cover}
                                    alt={`book-${book.id}`}
